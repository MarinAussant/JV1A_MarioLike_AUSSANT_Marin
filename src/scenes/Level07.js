...
class Level07 extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.image(0, 0, "backLevel07").setOrigin(0, 0).setScale(1).setPipeline('Light2D');
        ...
        //end
        this.mySkyglow = new JumpSkyglow(this, 5632, 1216);
        this.mySkyglow.setTint(0xf1b245);
        ...
        this.player.addOverlap(this.mySkyglow, this.endGame);
        ...
        // Particules de Saut
        this.jumpParticles = this.add.particles(this.player.x + 32 ,this.player.y - 2910 ,'jumpParticles', {
            emitZone:{
        ...
        this.add.image(0, 0, "frontLevel07").setOrigin(0, 0).setScale(1).setDepth(2).setPipeline('Light2D');
        
        // Particules de Speed

        this.speedParticles = this.add.particles(this.player.x + 32,this.player.y - 2910,'speedParticles', {
            emitZone:{
                source : new Phaser.Geom.Rectangle(this.player.x - 260, this.player.y - 225, 210, 275),
                type: "random",
            },
    ...
    //Creation de la map
    createMap() {
        const map = this.make.tilemap({ key: "level07" });
    ...
    onFallingPlatform(player,platform){
        ...
        if (platform.body.touching.down && player.body.touching.up && platform.onAir){
            player.respawn();
        }
    ...
    endGame(player, mySyglow){
        player.disableBody();
        player.cantMove = true;
        player.setState("idle");
        player.scene.lights.addLight(mySyglow.x, mySyglow.y, 150, 0xdd7815, 0.65);
        player.scene.cameras.main.fadeOut(3500, 34, 27, 29);
        ...
        player.scene.time.delayedCall(4000, () => {
            player.scene.sound.stopAll();
            player.scene.scene.start("Menu", {gamepad : haveGamepad});
